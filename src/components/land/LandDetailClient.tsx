...
import { landAPI, aiAPI, inquiryAPI } from '@/lib/api';
import { useAuthStore } from '@/store/authStore';
import toast from 'react-hot-toast';
import Image from 'next/image';
import Link from 'next/link';
import {
  MapPin, Brain, TrendingUp, Shield, BarChart3, CheckCircle, Heart,
  Eye, Phone, Mail, MessageCircle, Loader2, Star, AlertTriangle,
  Home, Zap, Droplets, Flame, Car, Wind, ArrowLeft
} from 'lucide-react';
...
export default function LandDetailClient({ slug }: { slug: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'analysis' | 'contact'>('overview');
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card overflow-hidden">
              <div className="relative h-80 bg-surface-hover">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  {land.isVerified && <span className="badge-green text-xs flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Verified</span>}
                </div>
                {hasAnalysis && (
                ...
                <div className="flex gap-2 p-3 overflow-x-auto">
                  {imgs.map((img: any, i: number) => (
                    <button key={i} onClick={() => setActiveImg(i)}
                      className={`w-16 h-12 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-all ${i === activeImg ? 'border-brand-500' : 'border-transparent'}`}
                    >
            ...
            <div className="flex gap-1 bg-surface-card border border-surface-border rounded-xl p-1">
              {(['overview', 'analysis', 'contact'] as const).map(tab => (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                ...
                <div className="card p-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {features.map(f => (
                      <div key={f.label} className={`flex items-center gap-2.5 p-3 rounded-xl border text-sm ${
                        f.active ? 'bg-brand-950 border-brand-900 text-brand-300' : 'bg-surface border-surface-border text-gray-600'
                      }`}>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {hasAnalysis ? (
                  ...
                  <div className="card p-10 text-center ai-gradient">
                    ...
                    <p className="text-gray-400 mb-6">Get a comprehensive AI report for this property including score, price projection, risk factors, and investment verdict.</p>
            ...
            <div className="card p-5 sticky top-20">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  {land.pricePerSqft && <p className="text-sm text-gray-500 mt-0.5">৳{land.pricePerSqft?.toLocaleString()} per sqft</p>}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="bg-surface rounded-xl p-3 border border-surface-border text-center">
                  <p className="font-bold text-white">{land.area?.toLocaleString()}</p>
                  <p className="text-xs text-gray-500">{land.areaUnit}</p>
                </div>
                <div className="bg-surface rounded-xl p-3 border border-surface-border text-center">
                  <p className="font-bold text-white capitalize">{land.landType}</p>
                  <p className="text-xs text-gray-500">Type</p>
                </div>
                <div className="bg-surface rounded-xl p-3 border border-surface-border text-center">
                  <p className="font-bold text-white">{land.views || 0}</p>
                  <p className="text-xs text-gray-500 flex items-center justify-center gap-1"><Eye className="w-3 h-3" /> Views</p>
                </div>
                <div className="bg-surface rounded-xl p-3 border border-surface-border text-center">
                  <p className="font-bold text-white">{land.inquiries || 0}</p>
                  <p className="text-xs text-gray-500 flex items-center justify-center gap-1"><MessageCircle className="w-3 h-3" /> Inquiries</p>
                </div>
              </div>
              ...
              {/* Run analysis CTA */}
              {!hasAnalysis && (
                <button onClick={handleAnalyze} disabled={analyzing} className="btn-outline w-full mt-4 text-sm">
